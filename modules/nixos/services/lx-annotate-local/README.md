...
| Unit | Type / trigger | Runtime role |
| --- | --- | --- |
| `lx-annotate-ffmpeg-stream-throttle.timer` | timer, default every 10 seconds | Starts `lx-annotate-ffmpeg-stream-throttle.service`, which asks Django whether user video streams are active and then applies runtime cgroup CPU/IO weights to the FFmpeg worker. Its last applied profile is stored in `/run/lx-annotate/ffmpeg-stream-throttle.state`. |
