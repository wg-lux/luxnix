    ...
    def validate_assessment(self) -> Assessment:
        ...
        if self.status is AssessmentStatus.NOT_ASSESSED:
            if (
                ...
                raise ValueError(
                    "not_assessed criteria cannot contain evidence or "
                    "assessment metadata",
                )
            return self
    ...
    def validate_checkpoint(self) -> WorkUnit:
        ...
        if self.status in {WorkUnitStatus.COMPLETE, WorkUnitStatus.BLOCKED}:
            if self.result is None or self.result.task_status != self.status.value:
                raise ValueError(
                    f"{self.status.value} work unit {self.id} requires a "
                    "matching result",
                )
...
class OrchestrationContract(BaseModel):
    ...
    def validate_orchestration(self) -> OrchestrationContract:
        ...
        if self.topology is TaskTopology.SEQUENTIAL_INTERDEPENDENT:
            ...
            if len(roots) < 2:
                raise ValueError(
                    "parallel topology requires at least two non-blocking "
                    "root work units",
                )
    ...
    *,
    ...
    with advisory_file_lock(lock_path=mutex_path, timeout_seconds=10):
        ...
        if message.recipient != owner:
            raise TrackerError(
                f"Agentennachricht {message_id} gehört '{message.recipient}', "
                f"nicht '{owner}'",
            )
...
def print_overview(
    ...
    *,
    ...
    print(
        f"Aktiv getrackt: {len(actively_tracked_features(features))}; "
        f"Done: {done_count}",
    )
    print(
        "Ein Score ersetzt nicht das Gate: Alle Pflichtkriterien müssen "
        "verifiziert sein.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if command == "orchestration":
        ...
        if orchestration_command == "validate":
            print(
                ...
                f"({contract.execution_mode.value}, "
                f"{len(contract.work_units)} Work-Units).",
            )
            return 0
    ...
    if command == "verify":
        ...
        if (
            ...
            raise TrackerError(
                f"{feature.id} ist done; vor Bewertungsänderungen zuerst "
                "reopen ausführen",
            )
