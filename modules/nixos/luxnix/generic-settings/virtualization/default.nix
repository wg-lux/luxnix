  ...
  config = mkIf cfg.enable {
    ...
    services = {
      ...
      virtualisation = {
        ...
        kvm.enable = mkIf cfg.kvm.enable false;
        vfio.enable = mkIf cfg.vfio.enable false;
      };
      udev.extraRules = mkIf cfg.vfio.enable ''
    ...
    boot = mkMerge [
      ...
      (mkIf cfg.vfio.enable {
        ...
        blacklistedKernelModules = optionals cfg.vfio.blacklistNvidia [
          "nvidia"
          "nouveau"
        ];
      })
    ...
    users = {
      ...
      users."qemu-libvirtd" = mkIf (cfg.kvm.enable && !cfg.kvm.qemu.runAsRoot) {
        extraGroups = [
          "kvm"
          "input"
        ];
      ...
      groups.${sensitiveServiceGroupName}.members = lib.mkIf (cfg.kvm.enable || cfg.vfio.enable) [
        username
      ];
    };
    ...
    systemd.tmpfiles.rules = mkIf cfg.vfio.enable (
      ...
      ++ (optional cfg.vfio.lookingGlass.enable "f /dev/shm/looking-glass 0660 ${username} kvm -")
    );
    ...
    warnings =
      (optional (
        cfg.vfio.enable && cfg.vfio.blacklistNvidia
      ) "VFIO: Nvidia drivers are blacklisted - GPU passthrough will be used")
      ++ (optional (
        cfg.vfio.hugepages.enable && cfg.vfio.hugepages.numPages > 2048
