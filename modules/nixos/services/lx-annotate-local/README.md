...
| `lx-annotate-terminology-bootstrap.service` | best-effort oneshot in wheel mode | After the web service starts, independently registers and activates the `report_template_examples` bundle shipped under the installed `lx_dtypes/data` package when no registry exists. No LX-Annotate startup unit wants, requires, or waits for this attempt. An existing registry is never reset. |
